...
ggplot(ozone, aes(x=vent)) + geom_bar() +xlab("") + ggtitle("vent")
...
Réponse : oui (on peut parler de la distribution des points)
...
Residuals: $\widehat Y$ chapeau - $Y$ coeff: contient les
estimations des coeff $\theta_0$ et $\theta_1$ ce qui nous donne les valeurs de $\hat \theta_0$ et $\hat \theta_1$
(for the rest, please check the picture you took of the whiteboard, date = 25/09/2024)
...
Réponse : represente les risidu pour chaque observation soit alors $Y_i - \widehat Y_i $
...
ggplot(ozone, aes(T12, maxO3))+
    ...
    xlab("T12")+  ylab("maxO3")
```
Interpretation de la zone en gris:
$x_0$ : nouvelle valeur pour T12
$Y_0 = \theta_0 + \theta_1 *x_0 + \varepsilon_0$ avec $\varepsilon_0$ suit la loi normale (0,$\sigma²$)
Intervalle de confiance de $\mathbb{E}[Y_0] = \theta_0 + \theta_1.x_0 $ =  la valeur moyenne de maxO3 quand T12 = $x_0$
...
```{r,eval=F}
autoplot(reg.simple,which=c(1,2,3,4),label.size=2)     
```
Interpretation des graphes:
graphe[1,1]: 
les abscisses sont les $\hat Y_i$ et les ordonnees sont les $\hat \varepsilon_i$
Hypothese 1 $\mathbb{E}[\varepsilon_i] = 0$ et $\mathbb{E}[Y_i] = \theta_0 + \theta_1.x_i $
pas tout a fait verifie donc amelioration possible avec toutes les variables

graphe[1,2]: 
Hypothese2 $\mathbb{var}[\varepsilon_i] = \sigma² $ 
cette hypothese sempble correcte 

graphe[1,1]:
Hypothese 3 Les $\varepsilon_i$ sont $iid$
on regarde s'il y'a des paquets de residus positives ou négatives
il semble que dans la zone autour de $\widehat {T12} \in [20,25]$ contradit cette hypothése
On peut voir sue le modéle suretimes la valeur de maxO3 de quelques points (voir le graphe avec le nuage de point et la droite de regression)

graphe[1,2]:
Hypothese 4 $\varepsilon_i$ suit la loi normale N(0, $\sigma²$)
L'hypothése de normalité semble etre verifiée

graphe[2,2]: Distance de cook
Pour chaque observation on regarde ce qu'on obtient pour $\theta$
detection des valeurs suspectes: celle ayant une valeur aberantes
...
Intervalle de prédiction pour $Y_0$ = valeur de maxO3 associé à Temp T12 = $x_0$
$\mathbb{IP}_{0.95}[Y_0] = [\hat Y_0 \pm  t_{0.975}.\hat \sigma .\sqrt{1 +v_0²}]$
...
```{r}
# A COMPLETER (deja fait dans la premiére partie)
summary(ozone1)
```
...
$$
Y_i = \theta_0 + \theta_1 .x_{i,1} + ... + \theta_n.x_{i,j} + \varepsilon_i
$$
...
```{r,eval = F}
# COMPLETER
autoplot(reg.mul, label.size = 2, which = 1:3)
```
graphe[2,1]: si on obtient une droite horizentale, on conclue que H2 est vérifiée
...
```{r,eval = F}
choix=regsubsets(maxO3 ~. , data = ozone1,nbest =1, nvmax = 11, method = "backward") # A COMPLETER
plot(choix, scale="Cp") #on veut choisir aussi au lieu de Cp les scales: bic,adjr2
plot(choix, scale="adjr2")
plot(choix, scale="bic")
stepAIC(reg.mul)
# A COMPLETER
```
On remarque que le modéle a gardé une valeur pour chaque catégorie.
...
```
On valide le sous-modéle à l'aide d'un test de Fisher, on retrouve que SSR...
...
```{r, eval=F}
ridge_cv <- cv.glmnet(tildeX, tildeY, alpha = 0, lambda = lambda_seq, nfolds = 10, type.measure = c("mse"), intercept = F) # A completer
